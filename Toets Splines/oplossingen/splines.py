...
def rechte(begin,eind):
    def koorde(x): return sp.Rational((eind[1]-begin[1])/(eind[0]-begin[0]))*(x-begin[0])+begin[1]
    return koorde
...
def parabool(begin,eind,rico):
    ...
    parameters = sp.solve([parabool(begin[0])-begin[1], parabool(eind[0])-eind[1], afgeleide(begin[0])-rico],a,b,c)
...
def optimaal(X,Y):
    ...
    S = 0
    for k in range(len(X)-1):
        S += sp.integrate( (parabolen[k](x)-rechten[k](x))**2, (x,X[k],X[k+1]) )
    print(S)
